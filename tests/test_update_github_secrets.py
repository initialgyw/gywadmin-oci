...
class TestGitHubSecrets:
    ...
    @pytest.mark.parametrize(
        "path_to_delete, expected_substring",
        [
    ...
    def test_from_summary_missing_required(
        self, valid_summary, path_to_delete, expected_substring
    ):
        data = valid_summary
...
class TestSetGithubSecret:
    def test_set_secret_success_argv_has_repo_flag(self, log):
        runner = FakeRunner()
        res = set_github_secret(
            "FOO", "barvalue", repo="owner/repo", runner=runner, log=log
        )
        assert res is True
    ...
    def test_set_secret_redacts_value_if_echoed(self, log, caplog):
        ...
        res = set_github_secret(
            "FOO", "super-secret", repo="owner/repo", runner=runner, log=log
        )
        assert res is False
...
class TestPreflightGh:
    ...
    def test_preflight_repo_failure_exits_4(self, log):
        runner = MultiRunner(
            [{"returncode": 0}, {"returncode": 1, "stderr": b"repo error"}]
        )
        with pytest.raises(SystemExit) as exc:
            preflight_gh("owner/repo", runner=runner, log=log)
        assert exc.value.code == 4
...
class TestProcessSecrets:
    ...
    def test_process_dry_run_logs_plan_with_names_only(self, summary_file, log, caplog):
        ...
        plan_line = next(
            ...
            if "Dry run: would set" in r.getMessage()
        )
    ...
    def test_process_partial_failure_best_effort_returns_1(
        self, summary_file, log, caplog
    ):
        runner = MultiRunner(
            [
                {"returncode": 0},
    ...
    def test_process_fail_fast_aborts(self, summary_file, log):
        runner = MultiRunner(
            [{"returncode": 0}, {"returncode": 0}, {"returncode": 0}, {"returncode": 1}]
        )
        res = process_secrets(
            summary_file,
            repo="owner/repo",
    ...
    def test_process_invalid_json_exits_3(self, tmp_path, log):
        ...
        with pytest.raises(SystemExit) as exc:
            process_secrets(
                p, repo="owner/repo", dry_run=False, yes=True, fail_fast=False, log=log
            )
        assert exc.value.code == 3
    ...
    def test_process_gh_missing_during_secrets_returns_2(
        self, summary_file, log, caplog
    ):
        def raise_fn(*args, **kwargs):
            raise FileNotFoundError()
    ...
    def test_process_user_declines_confirmation_exits_11(
        self, summary_file, log, monkeypatch
    ):
...
class TestMain:
    ...
    def test_main_propagates_exit_code_from_process_secrets(self, monkeypatch):
        ...
        monkeypatch.setattr(
            update_github_secrets, "process_secrets", lambda *args, **kwargs: 1
        )
